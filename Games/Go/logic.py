...
class GoGame:
    ...
    def __init__(self, size: int = 19):
        if size not in (9, 13, 19):
            raise ValueError("Go boards must be 9x9, 13x13, or 19x19 for this implementation.")
        self.size = size
        self.board: List[List[Optional[Color]]] = [[None for _ in range(size)] for _ in range(size)]
        ...
        self._snapshots: List[Tuple[List[List[Optional[Color]]], Color, Dict[Color, int]]] = [
    ...
    # Helpers
    # ------------------------------------------------------------------
    def _copy_board(self, board: Sequence[Sequence[Optional[Color]]]) -> List[List[Optional[Color]]]:
    ...
    def _collect_group(self, board: Sequence[Sequence[Optional[Color]]], start: Point) -> Tuple[Set[Point], int]:
    ...
    def play(self, row: int, col: int) -> None:
        ...
        self._snapshots.append((self._copy_board(self.board), self.to_move, self.captures.copy()))
    ...
    def _simulate_move(self, row: int, col: int, color: Color) -> Tuple[List[List[Optional[Color]]], int]:
    ...
    def _territory_map(self) -> List[List[Optional[Color]]]:
        territories: List[List[Optional[Color]]] = [[None for _ in range(self.size)] for _ in range(self.size)]
