...
class SoundBoard:
    ...
    def _synth_sound(self, frequency: float, duration: float, *, decay: float = 0.999) -> pygame.mixer.Sound:
        sample_rate = 44100
...
class City:
    ...
    def rect(self) -> pygame.Rect:
        return pygame.Rect(int(self.x - self.width / 2), int(SCREEN_HEIGHT - GROUND_HEIGHT - self.height), int(self.width), int(self.height))
...
class Base:
    ...
    def draw(self, surface: pygame.Surface) -> None:
        ...
        if self.capacity:
            ...
            bar_width = size
            bar_height = 8
            bar_rect = pygame.Rect(int(self.x - bar_width / 2), int(self.y + size / 2 + 6), bar_width, bar_height)
            pygame.draw.rect(surface, (45, 45, 65), bar_rect)
            fill_rect = pygame.Rect(bar_rect.left, bar_rect.top, int(bar_width * ammo_ratio), bar_height)
            pygame.draw.rect(surface, (120, 240, 255), fill_rect)


class Missile:
    def __init__(self, start: Tuple[float, float], target: Tuple[float, float], speed: float) -> None:
        self.x, self.y = start
    ...
    def has_reached_target(self) -> bool:
        if not self.active:
            return False
        reached_x = (self.vx >= 0 and self.x >= self.target_x) or (self.vx < 0 and self.x <= self.target_x)
        reached_y = (self.vy >= 0 and self.y >= self.target_y) or (self.vy < 0 and self.y <= self.target_y)
...
class Explosion:
    ...
    def draw(self, surface: pygame.Surface) -> None:
        if not self.active:
            return
        pygame.draw.circle(surface, (240, 200, 120), (int(self.x), int(self.y)), int(max(2, self.radius)), width=2)
...
class CooperativeCursor:
    ...
    def draw(self, surface: pygame.Surface) -> None:
        pygame.draw.circle(surface, PLAYER_TWO_CURSOR_COLOR, (int(self.x), int(self.y)), 6, width=1)
        pygame.draw.line(surface, PLAYER_TWO_CURSOR_COLOR, (int(self.x) - 10, int(self.y)), (int(self.x) + 10, int(self.y)))
        pygame.draw.line(surface, PLAYER_TWO_CURSOR_COLOR, (int(self.x), int(self.y) - 10), (int(self.x), int(self.y) + 10))


class MissileCommandGame:
    def __init__(self, *, two_player: bool = False, settings: Optional[DifficultySettings] = None) -> None:
    ...
    def reset_level(self) -> None:
        self.cities = [City(x, width=50, height=28) for x in np.linspace(100, SCREEN_WIDTH - 100, CITY_COUNT)]
        ...
        self.wave_to_spawn = self.settings.enemy_wave_size + self.settings.enemy_wave_growth * (self.level - 1)
        self.spawned_this_wave = 0
        self.spawn_timer = 0.0
        self.warning_played = False
    ...
    def spawn_enemy_missile(self) -> None:
        ...
        base_speed = self.settings.enemy_speed + self.settings.enemy_speed_growth * (self.level - 1)
    ...
    def handle_enemy_impacts(self) -> None:
        for missile in list(self.enemy_missiles):
            ...
            if missile.has_reached_target():
                ...
                self.explosions.append(Explosion(missile.target_x, missile.target_y, self.settings))
            ...
            for missile in self.enemy_missiles:
                if explosion.collides_with(missile):
                    if missile.active:
                        missile.active = False
                        self.sound.play_explosion()
                        self.score += int(self.settings.score_per_missile * self.score_multiplier)
        self.enemy_missiles = [m for m in self.enemy_missiles if m.active]
        self.explosions = [e for e in self.explosions if e.active]

    def fire_interceptor(self, target_x: float, target_y: float, *, player_two: bool = False) -> None:
        ...
        if base.fire():
            interceptor = Missile((base.x, base.y), (target_x, target_y), self.settings.interceptor_speed)
    ...
    def update_interceptors(self, dt: float) -> None:
        for interceptor in list(self.interceptors):
            ...
            if interceptor.has_reached_target():
                ...
                self.explosions.append(Explosion(interceptor.target_x, interceptor.target_y, self.settings))
                self.sound.play_explosion()
    ...
    def update(self, dt: float) -> None:
        ...
        spawn_max = max(spawn_min, self.settings.spawn_delay_range[1] * (1 - delay_decay))
    ...
    def check_level_complete(self) -> None:
        missiles_remaining = self.spawned_this_wave < self.wave_to_spawn or any(m.active for m in self.enemy_missiles)
        if missiles_remaining or self.interceptors or any(e.active for e in self.explosions):
            return
        surviving_cities = sum(1 for city in self.cities if city.alive)
        self.score += surviving_cities * self.settings.survival_bonus * int(self.score_multiplier)
        self.level += 1
        self.reset_level()
    ...
    def draw_hud(self) -> None:
        ammo_text = " | ".join(f"Base {idx+1}: {base.ammo}" for idx, base in enumerate(self.bases))
        hud_lines = [
            f"Level {self.level}",
            f"Score {self.score}",
    ...
    def process_events(self) -> None:
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if self.two_player_mode and event.key == pygame.K_SPACE and not self.game_over:
                    self.fire_interceptor(self.coop_cursor.x, self.coop_cursor.y, player_two=True)
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    parser.add_argument("--two-player", action="store_true", help="Start in cooperative mode with a keyboard-guided cursor.")
