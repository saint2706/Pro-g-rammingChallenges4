...
class LevelData:
    ...
    def from_file(cls, path: Path) -> "LevelData":
        ...
        return cls(
            ...
            ghost_starts={name: tuple(pos) for name, pos in payload["ghost_starts"].items()},
            scatter_targets={name: tuple(pos) for name, pos in payload["scatter_targets"].items()},
        )
...
class Pacman(Entity):
    ...
    def update(self, dt: float, level: LevelData, speed: float):
        ...
        if self.pending_direction is not None and self.can_move(self.pending_direction, level):
    ...
    def can_move(self, direction: Direction, level: LevelData) -> bool:
        ...
        return level.walkable((int(round(next_tile.x)), int(round(next_tile.y))), through_door=False)
...
class Ghost(Entity):
    ...
    def available_directions(self, level: LevelData) -> List[Direction]:
        ...
        for direction in DIRECTIONS:
            ...
            can_pass_door = self.state in (GhostState.EATEN, GhostState.CHASE, GhostState.SCATTER)
...
class ModeController:
    ...
    def update(self, dt: float) -> GhostState:
        ...
        if self.timer >= self.cycle[self.index]:
            self.timer = 0.0
            self.index += 1
            self.state = GhostState.CHASE if self.state == GhostState.SCATTER else GhostState.SCATTER
        return self.state
...
class Game:
    ...
    def load_assets(self):
        ...
        self.frightened_sprite = pygame.image.load(image_dir / "ghost_frightened.png").convert_alpha()
        self.eaten_sprite = pygame.image.load(image_dir / "ghost_eaten.png").convert_alpha()
    ...
    def load_level(self):
        ...
        modifiers = self.config.level_speed_modifiers[min(self.level_index, len(self.config.level_speed_modifiers) - 1)]
    ...
    def update(self, dt: float):
        ...
        for ghost in self.ghosts.values():
            speed = self.frightened_speed if ghost.state == GhostState.FRIGHTENED else self.ghost_speed
    ...
    def draw(self, screen: pygame.Surface):
        ...
        for tile in self.pellets:
            center = (int(tile[0] * tile_size + tile_size / 2), int(tile[1] * tile_size + tile_size / 2))
        ...
        screen.blit(sprite, sprite.get_rect(center=self.pacman.pixel_position(tile_size)))
        # Draw ghosts
        for ghost in self.ghosts.values():
            if ghost.state == GhostState.FRIGHTENED:
        ...
        hud_level = self.font.render(f"Level: {self.level_index + 1}", True, (255, 255, 255))
