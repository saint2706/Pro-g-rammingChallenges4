...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Plot metrics from multi-agent tag simulations"
    )
    parser.add_argument(
        "paths", type=Path, nargs="+", help="Recorded simulation JSON files"
    )
    parser.add_argument(
        "--output", type=Path, default=Path("analysis.png"), help="Output image path"
    )
