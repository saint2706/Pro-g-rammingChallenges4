...
def compute_layout(width: int, height: int) -> Tuple[pygame.Rect, pygame.Rect, pygame.Rect, pygame.Rect]:
    roster_rect = pygame.Rect(0, 0, int(width * 0.3), height - 120)
    log_rect = pygame.Rect(roster_rect.right, 0, width - roster_rect.width, int(height * 0.35))
...
def main() -> None:
    ...
    def breed_monster() -> str:
        ...
        return f"Egg incubating: {egg.species} (hatches in {egg.days_until_hatch} days)."
    ...
    def update_button_states() -> None:
        monster = current_monster()
        button_feed_basic.enabled = bool(monster) and available_feed_count(feed_quality) > 0
        button_feed_deluxe.enabled = bool(monster) and state.inventory.get("deluxe_feed", 0) > 0
        button_train.enabled = bool(monster)
        button_rest.enabled = bool(monster)
        button_breed.enabled = bool(monster) and state.select_best_partner(monster) is not None
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.button == 1:
                    new_index = roster_selection_from_pos(event.pos, roster_rect, len(state.monsters), font_medium)
                    if new_index is not None:
                        selected_index = new_index
                        status_message = f"Selected {state.monsters[selected_index].name}."
                    else:
...
def roster_selection_from_pos(
    ...
    for idx in range(count):
        entry_rect = pygame.Rect(rect.left + margin, y_offset, rect.width - margin * 2, entry_height)
...
def draw_roster(
    ...
    for idx, monster in enumerate(state.monsters):
        ...
        name_text = font_medium.render(f"{monster.name} ({monster.species.name})", True, TEXT_PRIMARY)
...
def draw_event_log(
    ...
    summary = font_medium.render(
        f"Day {state.day} | Gold: {state.gold} | Reputation: {state.reputation}", True, TEXT_PRIMARY
    )
    ...
    log_area = pygame.Rect(rect.left + 10, info_rect.bottom + 6, rect.width - 20, rect.height - info_height - 12)
...
def draw_details(
    ...
    if not state.monsters or selected_index < 0:
        placeholder = font_medium.render("Select a monster to view details", True, TEXT_MUTED)
        surface.blit(placeholder, placeholder.get_rect(center=rect.center))
        return
    monster = state.monsters[selected_index]
    header = font_large.render(f"{monster.name} — {monster.species.name}", True, TEXT_PRIMARY)
    ...
    surface.blit(subline, (rect.left + margin, rect.top + margin + header.get_height() + 4))

    stats_y = rect.top + margin + header.get_height() + 32
    for stat in STAT_NAMES:
        value = monster.stats[stat]
        stat_text = font_medium.render(f"{stat.capitalize()}: {value:.1f}", True, TEXT_PRIMARY)
    ...
    log_label = font_small.render(f"Last activity: {monster.last_event}", True, TEXT_MUTED)
...
def draw_action_bar(
    ...
    button_area = pygame.Rect(rect.left + 16, rect.top + 48, rect.width - 32, rect.height - 56)
    rows = 2
