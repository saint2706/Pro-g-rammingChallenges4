...
class GameState:
    ...
    gold: int = 120
    reputation: int = 0
    inventory: Dict[str, int] = field(default_factory=lambda: {"basic_feed": 8, "deluxe_feed": 2})
    ...
    def breed_monsters(self, parent_a: Monster, parent_b: Monster) -> Egg:
        ...
        dominant_species = parent_a.species if parent_a.level >= parent_b.level else parent_b.species
        stats = blend_offspring_stats(parent_a, parent_b, potential)
        nickname_hint = f"{dominant_species.name[:3]}-{parent_a.name[0]}{parent_b.name[0]}"
        egg = Egg(
    ...
    def _process_eggs(self) -> None:
        ...
        for egg in self.eggs:
            ...
            if egg.days_until_hatch <= 0:
                ...
                if not species:
                    self.log_event(f"Egg could not hatch: unknown species '{egg.species}'.")
                    continue
                stats = egg.stat_blueprint or species.base_stats
                name = egg.nickname_hint or f"New {species.name}"
                monster = Monster(name=name, species=species, stats=stats, stage="hatchling")
                hatched.append((egg, monster))
        for egg, monster in hatched:
            self.eggs.remove(egg)
            self.monsters.append(monster)
            parent_info = " & ".join(egg.parent_names or [])
            self.log_event(f"Egg hatched into {monster.species.name} {monster.name}! (Parents: {parent_info})")
    ...
    @classmethod
    def from_dict(cls, species_catalog: Dict[str, MonsterSpecies], data: Dict[str, object]) -> "GameState":
        state = cls(
            species_catalog=species_catalog,
            day=int(data.get("day", 1)),
            gold=int(data.get("gold", 0)),
            reputation=int(data.get("reputation", 0)),
            inventory={str(key): int(value) for key, value in (data.get("inventory") or {}).items()},
        )
        state.monsters = [Monster.from_dict(species_catalog, entry) for entry in data.get("monsters", [])]
    ...
    @classmethod
    def load(cls, species_catalog: Dict[str, MonsterSpecies], path: Path) -> "GameState":
