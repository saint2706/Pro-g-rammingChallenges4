...
class MonsterSpecies:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "MonsterSpecies":
        missing = {key for key in ("name", "element", "temperament", "base_stats", "growth_rates") if key not in data}
        if missing:
            raise ValueError(f"Species definition missing keys: {', '.join(sorted(missing))}")
        base_stats = {stat: int(data["base_stats"].get(stat, 0)) for stat in STAT_NAMES}
        growth_rates = {stat: float(data["growth_rates"].get(stat, 1.0)) for stat in STAT_NAMES}
        return cls(
            name=str(data["name"]),
...
class Monster:
    ...
    age_days: int = 0
    stage: str = "hatchling"
    stats: Dict[str, float] = field(default_factory=lambda: {stat: 0.0 for stat in STAT_NAMES})
    energy: float = 80.0
    hunger: float = 20.0
    ...
    def advance_day(self) -> List[str]:
        ...
        self.happiness = max(0.0, min(100.0, self.happiness - (3.0 if self.hunger > 70 else 0.5)))
    ...
    def train(self, focus: str) -> str:
        if focus not in STAT_NAMES:
            raise ValueError(f"Invalid focus '{focus}'. Expected one of {', '.join(STAT_NAMES)}.")
    ...
    @classmethod
    def from_dict(cls, species_catalog: Dict[str, MonsterSpecies], data: Dict[str, Any]) -> "Monster":
        ...
        return cls(
            ...
            stats={stat: float(data.get("stats", {}).get(stat, species_catalog[species_name].base_stats.get(stat, 0))) for stat in STAT_NAMES},
...
class Egg:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "stat_blueprint": dict(self.stat_blueprint) if self.stat_blueprint else None,
        }
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "Egg":
        return cls(
            ...
            stat_blueprint={stat: float(value) for stat, value in (data.get("stat_blueprint") or {}).items()} or None,
        )
...
DEFAULT_SPECIES_DATA: List[Dict[str, Any]] = [
    {
        ...
        "base_stats": {"strength": 18, "intelligence": 16, "endurance": 20, "agility": 22, "charisma": 24},
        "growth_rates": {"strength": 1.1, "intelligence": 1.2, "endurance": 1.3, "agility": 1.4, "charisma": 1.2},
    },
    {
        "name": "Embercub",
        "element": "Fire",
        "temperament": "brash",
        "base_stats": {"strength": 24, "intelligence": 14, "endurance": 22, "agility": 20, "charisma": 18},
        "growth_rates": {"strength": 1.5, "intelligence": 1.0, "endurance": 1.2, "agility": 1.3, "charisma": 1.1},
    },
    {
        "name": "Starlet",
        "element": "Light",
        "temperament": "focused",
        "base_stats": {"strength": 16, "intelligence": 26, "endurance": 18, "agility": 20, "charisma": 28},
        "growth_rates": {"strength": 1.0, "intelligence": 1.6, "endurance": 1.1, "agility": 1.2, "charisma": 1.5},
    },
    {
        "name": "Tidewhirl",
        "element": "Water",
        "temperament": "calm",
        "base_stats": {"strength": 20, "intelligence": 22, "endurance": 24, "agility": 18, "charisma": 19},
        "growth_rates": {"strength": 1.2, "intelligence": 1.3, "endurance": 1.5, "agility": 1.0, "charisma": 1.1},
    },
]
...
def create_monster(species: MonsterSpecies, nickname: Optional[str] = None) -> Monster:
    ...
    stats = {
        stat: float(value + random.uniform(-2.0, 2.0)) for stat, value in species.base_stats.items()
    }
    name = nickname or generate_monster_name(species)
    return Monster(name=name, species=species, stats=stats)


def blend_offspring_stats(parent_a: Monster, parent_b: Monster, potential: float) -> Dict[str, float]:
