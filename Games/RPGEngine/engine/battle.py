...
class BattleScene:
    ...
    def _draw_status(self, surface: pygame.Surface) -> None:
        player_text = self.font.render(
            f"{self.player.name} HP: {self.player.stats.hp}/{self.player.stats.max_hp}", True, (255, 255, 255)
        )
        enemy_text = self.font.render(
            f"{self.enemy.name} HP: {self.enemy.stats.hp}/{self.enemy.stats.max_hp}", True, (255, 120, 120)
        )
    ...
    def _use_item(self) -> None:
        consumables = [item_id for item_id in self.player.inventory.list_items() if self.items[item_id].heal_amount > 0]
        if not consumables:
...
def create_enemy_from_data(enemy_data: Dict[str, object]) -> Entity:
    ...
    return Entity(entity_id=str(enemy_data["id"]), name=str(enemy_data["name"]), stats=stats)
