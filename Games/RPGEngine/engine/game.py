...
class Game:
        ...
        for dialogue_id, nodes in dialogue_data.items():
            parsed_nodes = {
                node_id: DialogueLine(
                    ...
                    complete_objectives={
                        qid: list(indices)
                        for qid, indices in node_info.get(
                            "complete_objectives", {}
                        ).items()
                    },
                    reward_objectives={
    ...
    def get_nearby_npc(self) -> NPC | None:
        ...
        for npc in self.npcs:
            if (
                abs(npc.position[0] - self.player.x)
                + abs(npc.position[1] - self.player.y)
                == 1
            ):
                return npc
        return None
    ...
    def draw_dialogue(self) -> None:
        ...
        text = self.font.render(
            f"{self.dialogue_node.speaker}: {self.dialogue_node.text}",
            True,
            (255, 255, 255),
        )
    ...
    def start_battle(self, enemy_id: str) -> None:
        ...
        self.battle_scene = BattleScene(
            self.player_entity, create_enemy_from_data(enemy_data), self.items
        )
        self.state = "battle"
        self.space_down = True
        ...
        if self.battle_scene.ended:
            ...
            if keys[pygame.K_SPACE] and not self.space_down:
                ...
                if (
                    ...
                    and resume_tree
                    and resume_id
                    and resume_id in resume_tree.nodes
                ):
