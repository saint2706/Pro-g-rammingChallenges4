...
class Renderer:
    ...
    def _draw_ghost(self, cells: Iterable[Tuple[int, int]]) -> None:
        ghost_surface = pygame.Surface(self.settings.playfield_pixel_size, pygame.SRCALPHA)
        color = (*self.board.active.color, GHOST_ALPHA) if self.board.active else (255, 255, 255, GHOST_ALPHA)
        block = self.settings.block_size
        for x, y in cells:
    ...
    def _draw_queue(self, origin: Tuple[int, int]) -> None:
        ...
        for idx, name in enumerate(self.board.peek_queue()[:self.settings.next_queue_length]):
            shape = PIECES[name]["rotations"][0]
            color = PIECES[name]["color"]
            offset_x = 20
            offset_y = idx * 60 + 10
            for x, y in shape:
                rect = pygame.Rect(origin_x + offset_x + x * block, origin_y + offset_y + y * block, block, block)
                pygame.draw.rect(self.surface, color, rect)
                pygame.draw.rect(self.surface, GRID_COLOR, rect, 1)

    def _draw_text(self, text: str, position: Tuple[int, int], font: pygame.font.Font | None = None) -> None:
