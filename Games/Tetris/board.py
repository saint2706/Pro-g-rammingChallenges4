...
class Piece:
    ...
    def cells(self, rotation: Optional[int] = None, offset: Optional[Position] = None) -> List[Position]:
...
class TetrisBoard:
    ...
    def spawn_piece(self) -> None:
        ...
        x = self.width // 2 - 2
        y = 0
        piece = Piece(name=name, rotations=data["rotations"], color=data["color"], x=x, y=y)
    ...
    def move(self, dx: int) -> bool:
        if not self.active or dx == 0 or self.game_over:
            return False
        if self._is_valid_position(self.active, self.active.x + dx, self.active.y, self.active.rotation_index):
            self.active.x += dx
            return True
        return False
    ...
    def rotate(self, direction: int) -> bool:
        if not self.active or self.game_over:
            return False
        new_index = (self.active.rotation_index + direction) % len(self.active.rotations)
    ...
    def soft_drop(self) -> bool:
        if not self.active or self.game_over:
            return False
        if self._is_valid_position(self.active, self.active.x, self.active.y + 1, self.active.rotation_index):
            self.active.y += 1
            self.score += 1
            return False
        self.lock_piece()
    ...
    def hard_drop(self) -> None:
        if not self.active or self.game_over:
            return
        distance = 0
        while self._is_valid_position(self.active, self.active.x, self.active.y + 1, self.active.rotation_index):
            self.active.y += 1
            distance += 1
    ...
    def tick_gravity(self) -> None:
        if not self.active or self.game_over:
            return
        if not self._is_valid_position(self.active, self.active.x, self.active.y + 1, self.active.rotation_index):
            self.lock_piece()
        else:
            self.active.y += 1
    ...
    def _drop_distance(self) -> int:
        ...
        while self._is_valid_position(
            self.active, self.active.x, self.active.y + distance + 1, self.active.rotation_index
        ):
            distance += 1
        return distance
