...
class GameMap:
    ...
    def get_blocking_entity_at_location(self, dest_x: int, dest_y: int) -> Optional["Entity"]:
    ...
    def serialise_entity(self, entity: "Entity") -> Dict[str, object]:
        ...
        if getattr(entity, "fighter", None):
            ...
            data["fighter"] = {"hp": fighter.hp, "max_hp": fighter.max_hp, "defense": fighter.defense, "power": fighter.power}
        if getattr(entity, "inventory", None):
            data["inventory"] = {
                "capacity": entity.inventory.capacity,
                "items": [self.serialise_entity(item) for item in entity.inventory.items],
            }
        if getattr(entity, "consumable", None):
            consumable = entity.consumable
            serialisable = {key: value for key, value in consumable.__dict__.items() if key != "parent"}
