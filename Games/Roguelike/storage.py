...
def save_game(engine: "Engine", path: str) -> None:
    data = {
        ...
        "entities": [engine.game_map.serialise_entity(e) for e in engine.game_map.entities if e is not engine.player],
...
def load_game(path: str) -> "Engine":
    ...
    for message_payload in payload.get("messages", []):
        engine.message_log.add(message_payload["text"], tuple(message_payload["colour"]))
...
def build_entity(data: Dict[str, object]):
    ...
    if entity_class == "Item":
        ...
        if consumable_payload:
            ...
            if cls:
                kwargs = {k: v for k, v in consumable_payload.items() if k not in {"type"}}
