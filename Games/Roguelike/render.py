...
def render_all(console: tcod.console.Console, engine: "Engine") -> None:
    ...
    entities_in_render_order = sorted(game_map.entities, key=lambda e: e.render_order.value)
...
def render_ui(console: tcod.console.Console, engine: "Engine") -> None:
    ...
    draw_message_log(console, log, start_y=y_offset + 2, height=config.screen_height - (y_offset + 2))

    draw_inventory(console, player.inventory, x=config.map_width - 25, y=y_offset)


def draw_message_log(console: tcod.console.Console, log: MessageLog, *, start_y: int, height: int) -> None:
