...
def test_download_roundtrip(server_root: Path, client_dir: Path) -> None:
    ...
    with TFTPServer(server_root, host="127.0.0.1", port=0, timeout=1.0, retries=5) as server:
        client = TFTPClient("127.0.0.1", port=server.server_port, timeout=1.0, retries=5)
