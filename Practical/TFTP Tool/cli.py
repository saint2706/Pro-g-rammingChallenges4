...
def run_server(args: argparse.Namespace) -> int:
    ...
    try:
        server.start()
        logging.getLogger(__name__).info("Server ready on %s:%s", args.host, server.server_port)
        while True:
            time.sleep(1.0)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--port", type=int, default=DEFAULT_PORT, help="UDP port (default: %(default)s)")
    parser.add_argument("--timeout", type=float, default=3.0, help="Socket timeout in seconds (default: %(default)s)")
    parser.add_argument("--retries", type=int, default=5, help="Retransmission attempts before aborting")
    parser.add_argument("-v", "--verbose", action="count", default=0, help="Increase log verbosity (repeat for debug)")
    ...
    server_parser.add_argument("--host", default="0.0.0.0", help="Bind address (default: %(default)s)")
    ...
    get_parser.add_argument("--blksize", type=int, default=512, help="Request block size (default: %(default)s)")
    ...
    put_parser.add_argument("--blksize", type=int, default=512, help="Request block size (default: %(default)s)")
