...
def render() -> None:
    """Render the Streamlit UI for the matrix arithmetic toolkit."""
    ...
    with st.form("matrix_form", clear_on_submit=False):
        ...
        visualize = st.checkbox(
            "Show 2×2 visualisation of the resulting matrix", value=False
        )
    ...
    if result.steps:
        ...
        text_lines.extend(
            f"{idx}. {step}" for idx, step in enumerate(result.steps, start=1)
        )
    st.download_button(
