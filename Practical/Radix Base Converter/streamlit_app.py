...
def _build_batch_downloads(rows: list[dict[str, str | int]]) -> tuple[bytes, bytes]:
    ...
    writer = csv.DictWriter(
        csv_buffer, fieldnames=["input", "from_base", "to_base", "output"]
    )
    writer.writeheader()
...
def render() -> None:
    st.set_page_config(
        page_title="Radix Base Converter", page_icon="🔢", layout="centered"
    )
    ...
    with col_to:
        to_base = int(
            st.number_input("To base", min_value=2, max_value=36, value=10, step=1)
        )
    with col_pad:
        pad_width = int(
            st.number_input("Pad width", min_value=0, max_value=64, value=0, step=1)
        )
    ...
    if st.button("Run batch conversion"):
        if not value.strip():
            ...
            try:
                ...
                if not results:
                    st.info(
                        "No valid conversions were produced for the provided bases."
                    )
                else:
                    table_rows: list[dict[str, str | int]] = []
                    for item in results:
                        formatted = format_output(
                            item.output, lower=lower_case, pad=pad_width
                        )
                        table_rows.append(
                            {
                                "input": item.input,
