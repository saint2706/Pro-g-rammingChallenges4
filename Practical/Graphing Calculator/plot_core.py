...
def sympify_expressions(functions: Sequence[str]) -> List[sp.Expr]:
    ...
    for expr in functions:
        try:
            ...
            sp.SympifyError,
            TypeError,
        ) as exc:  # pragma: no cover - depends on input
...
def evaluate_functions(
    functions: Sequence[str], settings: PlotSettings
) -> PlotComputation:
    ...
    return PlotComputation(
        x=x_vals, lines=lines, derivative_lines=derivative_lines, errors=errors
    )
