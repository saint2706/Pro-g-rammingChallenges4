...
public class pcSem {
   ...
   */
  /**
   * A bounded FIFO buffer implemented on top of an array list, coordinated via three semaphores:
   * emptySlots, filledSlots, and a binary mutex. This mirrors the classical Dijkstra solution while
  ...
  static final class BoundedBuffer<T> {
    ...
    void put(T item) throws InterruptedException {
      if (shutdown) return; // ignore late puts after shutdown
      emptySlots.acquire();
      mutex.acquire();
      try {
    ...
    public void run() {
      try {
        for (int i = 0; i < cfg.itemsPerProducer; i++) {
          ...
          if (cfg.prodDelayMs > 0) TimeUnit.MILLISECONDS.sleep(cfg.prodDelayMs);
        }
    ...
    public void run() {
      try {
        while (true) {
          ...
          if (cfg.consDelayMs > 0) TimeUnit.MILLISECONDS.sleep(cfg.consDelayMs);
        }
      } catch (InterruptedException ie) {
        Thread.currentThread().interrupt();
