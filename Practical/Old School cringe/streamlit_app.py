...
def _resolve_effect_class(
    factory: Callable[[retro.DemoConfig], retro.Effect],
) -> Optional[type[retro.Effect]]:
...
def render() -> None:
    ...
    for seg in segments:
        ...
        with st.expander(header, expanded=True):
            ...
            st.write(
                f"Fade-in: {seg['fade_in']:.1f}s · Fade-out: {seg['fade_out']:.1f}s"
            )
