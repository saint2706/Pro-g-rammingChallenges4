...
def _initialise_state(templates: Sequence[str]) -> None:
    ...
    if "markdown_text" not in st.session_state:
        st.session_state.markdown_text = (
            autosaved.get("content")
            if autosaved and autosaved.get("content")
...
def render() -> None:
    ...
    with st.sidebar:
        ...
        if template_names:
            ...
            st.info(
                "Drop HTML template files into the templates/ folder to enable rendering."
            )
    ...
    with preview_col:
        ...
        if not template_names:
            st.warning(
                "No templates found. The preview will show raw Markdown conversion without template styling."
            )
    ...
    with editor_col:
        if (
            ...
            and result.template_used != st.session_state.template_name
        ):
            st.info(
                "Front matter requested template '%s'. Rendering honours that selection."
                % result.template_used
            )
