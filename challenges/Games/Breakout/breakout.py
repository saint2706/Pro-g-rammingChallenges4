...
class Particle:
    ...
    def draw(self, surface: pygame.Surface) -> None:
        ...
        surface.blit(
            gfx_surface, (self.position.x - radius_px, self.position.y - radius_px)
        )
...
class Ball:
    ...
    def draw(self, surface: pygame.Surface) -> None:
        ...
        pygame.draw.circle(
            surface,
            (255, 255, 255),
            (int(self.position.x - 2), int(self.position.y - 2)),
            self.radius - 4,
        )
...
class Brick(pygame.sprite.Sprite):
    ...
    def _render(self) -> None:
        ...
        pygame.draw.rect(
            self.image, (236, 240, 241), self.image.get_rect(), 2, border_radius=4
        )
...
class BreakoutGame:
    ...
    def __init__(self) -> None:
        ...
        if not self.levels:
            raise SystemExit(
                "No level files found. Ensure JSON layouts exist in levels/."
            )
    ...
    def _build_sounds(self) -> Dict[str, pygame.mixer.Sound]:
        def tone(
            freq: float, duration: float = 0.2, volume: float = 0.3
        ) -> pygame.mixer.Sound:
            sample_rate = 44100
    ...
    def _paddle_collision(self, ball: Ball) -> None:
        ...
        if ball.velocity.y > 0 and ball.rect.colliderect(self.paddle.rect):
            ...
            rel = (ball.position.x - self.paddle.rect.centerx) / (
    ...
    def _spawn_particles(self, rect: pygame.Rect) -> None:
        ...
        for _ in range(10):
            velocity = pygame.Vector2(
                random.uniform(-120, 120), random.uniform(-200, 20)
            )
            particle = Particle(
                centre.copy(), velocity, colour, random.uniform(4, 8), 0.5
            )
    ...
    def _game_over(self) -> None:
        ...
        self.screen.blit(
            message, message.get_rect(center=(WIDTH // 2, HEIGHT // 2 - 40))
        )
        self.screen.blit(
            score_text, score_text.get_rect(center=(WIDTH // 2, HEIGHT // 2 + 10))
        )
        pygame.display.flip()
