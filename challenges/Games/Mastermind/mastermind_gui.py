...
class MastermindApp(tk.Tk):
    ...
    def _build_controls(self) -> None:
        ...
        peg_spin = ttk.Spinbox(
            frame,
            from_=3,
            to=8,
            textvariable=self.peg_var,
            width=5,
            command=self._update_guess_inputs,
        )
        peg_spin.grid(row=1, column=1, sticky="w")

        ttk.Label(frame, text="Colours:").grid(row=1, column=2, sticky="w")
        color_spin = ttk.Spinbox(
            frame,
            from_=4,
            to=12,
        ...
        ttk.Button(frame, text="Start new game", command=self.start_game).grid(
    ...
    def _build_guess_area(self) -> None:
        ...
        self.submit_button = ttk.Button(
            frame, text="Submit Guess", command=self.submit_guess, state="disabled"
        )
    ...
    def _build_secret_controls(self) -> None:
        ...
        self.set_secret_button = ttk.Button(
            frame, text="Set Secret", command=self.set_secret, state="disabled"
        )
    ...
    def _build_history(self) -> None:
        ...
        scrollbar = ttk.Scrollbar(
            frame, orient="vertical", command=self.history_list.yview
        )
    ...
    def _update_secret_inputs(self) -> None:
        ...
        palette = (
            ...
            if self.config_obj
            else generate_palette(self.color_var.get())
        )
        for idx in range(self.peg_var.get()):
            cb = ttk.Combobox(
                self.secret_frame,
    ...
    def _append_history(self, turn: int, guess: Sequence[str], score: Score) -> None:
        ...
        self.history_list.insert(
            tk.END, f"Turn {turn:02d}: {guess_text:<30} | {feedback}"
        )
