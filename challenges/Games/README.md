...
Authoritative map of every folder under `challenges/Games/` and its `/g/` challenge number. Each entry links to the implementation, summarises the shipped features, and lists the primary technologies in use.
...
Commands assume you are inside `challenges/Games/` with your virtual environment (if any) activated. Every row links to the detailed game README for controls, assets, and extended notes. Use the automated check column to quickly smoke-test builds that ship pytest coverage.
...
| [Roguelike](Roguelike/README.md) | `python -m challenges.Games.Roguelike.main` | tcod controls + save files | `pytest tests/test_ai_roguelike_smoke.py` |
