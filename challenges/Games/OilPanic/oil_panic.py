...
DIFFICULTIES: Dict[str, DifficultySettings] = {
    "Easy": DifficultySettings(
        "Easy", drop_interval=1.6, drop_speed=55.0, policeman_speed=28.0
    ),
    "Normal": DifficultySettings(
        "Normal", drop_interval=1.2, drop_speed=70.0, policeman_speed=35.0
    ),
    "Hard": DifficultySettings(
        "Hard", drop_interval=0.85, drop_speed=90.0, policeman_speed=46.0
    ),
}
...
def load_config() -> Tuple[int, str]:
    ...
    if not CONFIG_PATH.exists():
        CONFIG_PATH.write_text(
            json.dumps({"window_scale": 2, "difficulty": "Normal"}, indent=2)
        )
    try:
    ...
    return max(1, min(scale, 5)), (
        difficulty_name if difficulty_name in DIFFICULTIES else "Normal"
    )
...
def make_tone(
    frequency: float, duration: float = 0.18, volume: float = 0.45
) -> pygame.mixer.Sound:
...
class OilPanicGame:
    ...
    def reset(self) -> None:
        ...
        self.policeman = Policeman(
            bounds=(28, BASE_RESOLUTION[0] - 40), speed=self.difficulty.policeman_speed
        )
    ...
    def handle_input(self, dt: float) -> None:
        ...
        if (
            ...
            and self.bucket_contents > 0
        ):
            self.pour_oil()

    def pour_oil(self) -> None:
        policeman_in_zone = (
            self.deposit_zone[0] <= self.policeman.x <= self.deposit_zone[1]
        )
    ...
    def draw_top_screen(self) -> None:
        ...
        pygame.draw.rect(
            self.base_surface,
            (18, 28, 45),
            (0, TOP_SCREEN_HEIGHT - 10, BASE_RESOLUTION[0], 10),
        )
        # Draw pipes and windows reminiscent of the LCD backdrop
        for idx, x in enumerate(self.bucket_positions):
            pygame.draw.rect(
                self.base_surface, (80, 90, 120), (x - 10, 32, 20, 24), border_radius=6
            )
            pygame.draw.rect(
                self.base_surface, (140, 160, 200), (x - 8, 34, 16, 12), border_radius=4
            )
            pygame.draw.rect(self.base_surface, (40, 50, 80), (x - 4, 46, 8, 40))
        # Draw oil drops
        for drop in self.drops:
            pygame.draw.circle(
                self.base_surface, (24, 220, 180), (int(drop.x), int(drop.y)), 4
            )
            pygame.draw.circle(
                self.base_surface,
                (4, 120, 90),
                (int(drop.x) - 1, int(drop.y) - 1),
                4,
                1,
            )
        ...
        pygame.draw.rect(
            self.base_surface,
            (90, 90, 100),
            (bucket_x - 16, base_y - 30, 32, 26),
            border_radius=8,
        )
        pygame.draw.circle(
            self.base_surface, (230, 230, 210), (bucket_x, base_y - 34), 10
        )
        pygame.draw.rect(
            self.base_surface, (60, 60, 90), (bucket_x - 22, base_y - 12, 44, 12)
        )
        # Bucket
    ...
    def draw_bottom_screen(self) -> None:
        ...
        if self.active_screen == "bottom":
            ...
            fill_ratio = (
                self.bucket_contents / self.bucket_capacity
    ...
    def draw_hud(self) -> None:
        ...
        for idx, text in enumerate(instructions):
            ...
            self.base_surface.blit(
                surface, (BASE_RESOLUTION[0] - 150, TOP_SCREEN_HEIGHT + 96 + idx * 16)
            )
        if self.message:
        ...
        if self.game_over:
            ...
            self.base_surface.blit(
                text,
                text.get_rect(
                    center=(BASE_RESOLUTION[0] // 2, BASE_RESOLUTION[1] // 2 - 20)
                ),
            )
            ...
            self.base_surface.blit(
                prompt,
                prompt.get_rect(
                    center=(BASE_RESOLUTION[0] // 2, BASE_RESOLUTION[1] // 2 + 12)
                ),
            )
