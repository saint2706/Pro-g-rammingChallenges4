...
class Terrain:
    ...
    def generate(cls, width: int, height: int, resolution: int) -> "Terrain":
        ...
        for i in range(0, width, resolution):
            ...
            points.append(
                int(min(height - 40, max(height - 200, base + noise + jitter)))
            )
...
class Menu:
    ...
    def handle_input(self, input_mgr: InputManager) -> None:
        ...
        if input_mgr.was_pressed(pygame.K_UP):
            ...
            self.selected = (self.selected + 1) % len(self.options)
        elif input_mgr.was_pressed(pygame.K_RETURN) or input_mgr.was_pressed(
            pygame.K_SPACE
        ):
    ...
    def draw(self, surface: pygame.Surface) -> None:
        ...
        for idx, option in enumerate(self.options):
            ...
            surface.blit(
                text_surface,
                (WIDTH // 2 - text_surface.get_width() // 2, 180 + idx * 40),
            )
...
class GameApp:
    ...
    def run_weapon_menu(self) -> None:
        ...
        while self.state == "weapon_config":
            ...
            if self.input.was_pressed(pygame.K_RIGHT):
                ...
                attribute_index = (attribute_index - 1) % len(attributes)
            elif self.input.was_pressed(pygame.K_MINUS) or self.input.was_pressed(
                pygame.K_KP_MINUS
            ):
                self.modify_weapon(
                    self.weapons[selected], attributes[attribute_index], -1
                )
            elif self.input.was_pressed(pygame.K_EQUALS) or self.input.was_pressed(
                pygame.K_KP_PLUS
            ):
                self.modify_weapon(
                    self.weapons[selected], attributes[attribute_index], 1
            ...
            self.draw_footer(
                "Up/Down select weapon, Left/Right attribute, +/- adjust, S save, R reset, Esc back"
            )
            pygame.display.flip()
            self.clock.tick(60)
    ...
    def setup_game(self) -> None:
        total_players = (
            self.match_settings["players"] + self.match_settings["ai_players"]
        )
        ...
        for i in range(total_players):
            ...
            tank = Tank(
                name=(
                    f"AI {i - self.match_settings['players'] + 1}"
                    if is_ai
                    else f"Player {i + 1}"
                ),
    ...
    def handle_player_turn(self, tank: Tank) -> None:
        ...
        if keys[pygame.K_UP]:
            tank.power = min(
                MAX_POWER, tank.power + 80 * self.clock.get_time() / 1000.0
            )
    ...
    def simulate_shot_landing(
        self, tank: Tank, angle_rad: float, power: float
    ) -> float:
    ...
    def resolve_explosion(self, projectile: Projectile) -> None:
        ...
        for tank in self.tanks:
            ...
            if distance <= weapon.explosion_radius:
                damage_ratio = (
                    1 - (distance / weapon.explosion_radius) ** weapon.splash_falloff
                )
        ...
        for tank in self.tanks:
            ...
            pygame.draw.rect(
                ...
                pygame.Rect(
                    tank.rect().left,
                    tank.rect().top - 8,
                    int(TANK_WIDTH * health_ratio),
                    6,
                ),
            )
        if self.projectile:
            pygame.draw.circle(
                self.screen, (250, 250, 250), self.projectile.position, 4
            )
        hud_lines = [
    ...
    def run_scoreboard(self) -> None:
        while self.state == "scoreboard":
            self.input.update()
            if self.input.was_pressed(pygame.K_RETURN) or self.input.was_pressed(
                pygame.K_ESCAPE
            ):
                self.state = "main_menu"
                break
            ...
            sorted_tanks = sorted(
                self.tanks,
                key=lambda t: (t.health > 0, t.score, t.total_damage),
                reverse=True,
            )
