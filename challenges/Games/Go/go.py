...
class GoBoardWidget(QWidget):
    ...
    def paintEvent(self, event):  # type: ignore[override]
        ...
        painter.drawRect(
            left - cell / 2, top - cell / 2, board_size + cell, board_size + cell
        )
        ...
        if self._game.moves:
            ...
            if last.point:
                ...
                painter.setPen(Qt.NoPen)
                painter.drawEllipse(
                    QPointF(center_x, center_y), cell * 0.15, cell * 0.15
                )
...
class GoMainWindow(QMainWindow):
    ...
    def handle_move(self, row: int, col: int) -> None:
        ...
        self._update_status(
            f"{self.game.moves[-1].color} played at ({row + 1}, {col + 1})."
        )
        self._maybe_run_ai()
    ...
    def save_sgf(self) -> None:
        filename, _ = QFileDialog.getSaveFileName(
            self, "Save SGF", str(Path.home() / "game.sgf"), "Smart Game Format (*.sgf)"
        )
        if not filename:
            return
        try:
