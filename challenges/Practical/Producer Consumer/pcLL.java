...
/**
 * Modern producer-consumer example using intrinsic locks (synchronized) and wait/notifyAll.
 * Demonstrates a bounded FIFO with graceful shutdown.
 *
 * <p>Differences vs pcSem: - Uses intrinsic monitor methods (wait/notifyAll) instead of Semaphores.
 * - Explicit predicate checks inside while loops (handles spurious wakeups). - Employs a
 * poison-pill sentinel to stop consumers.
 *
 * <p>CLI args use key=value form. Example: java pcLL producers=2 consumers=2 itemsPerProducer=10
 * capacity=5 prodDelayMs=50 consDelayMs=80
 */
public class pcLL {
  /*
   * ------------------------------- Data Classes ------------------------------
  ...
  static final class BoundedQueue<T> {
    ...
    public synchronized void put(T item) throws InterruptedException {
      ...
      if (shutdown) return; // reject puts after shutdown
      list.addLast(item);
    ...
    public synchronized T take() throws InterruptedException {
      ...
      if (list.isEmpty()) return null; // shutdown & empty
    ...
    public void run() {
      try {
        for (int i = 0; i < cfg.itemsPerProducer; i++) {
          ...
          if (cfg.prodDelayMs > 0) Thread.sleep(cfg.prodDelayMs);
        }
  ...
  static final class Consumer implements Runnable {
    ...
    public void run() {
      try {
        while (true) {
          Object obj = q.take();
          if (obj == null) break; // shutdown & empty
          if (obj == POISON_PILL) {
          ...
          if (cfg.consDelayMs > 0) Thread.sleep(cfg.consDelayMs);
        }
      } catch (InterruptedException ie) {
        Thread.currentThread().interrupt();
  ...
  public static void main(String[] args) throws InterruptedException {
    ...
    for (Thread t : threads) {
      if (t.getName().startsWith("Producer-")) t.join();
    }
    // Insert poison pills
    ...
    // Join consumers
    for (Thread t : threads) {
      if (t.getName().startsWith("Consumer-")) t.join();
    }
    ...
    System.out.println(
        "  Producers:        " + cfg.producers + " (each " + cfg.itemsPerProducer + ")");
