...
public:
    ...
    Image(int sx, int sy)
        : SizeX(sx), SizeY(sy), Pixels(new Intens[sx * sy])
    {
    }
...
static void insertKeyedPoint(KeyedPoint pts[], int count, Point pt, float key, float temp = 0.0f)
{
    int j;
...
static void cellularTexSortY_SSE(Image &out, const Point ptIn[], int count)
{
    ...
    for (int y = 0; y < out.SizeY; y++)
    {
        ...
        static const __declspec(align(16)) unsigned int clearSignC[4] = {0x7fffffff, 0x7fffffff, 0x7fffffff, 0x7fffffff};

        __m128 curX = _mm_setr_ps(stepX * 0.0f, stepX * 1.0f, stepX * 2.0f, stepX * 3.0f);
        ...
        for (int x = 0; x < out.SizeX; x += 4)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                __m128 cmp = _mm_cmpgt_ps(best2, ptKey);     // not done for any of the pixels?
...
static void cellularTexTilesRect(Image &out, int x0, int y0, int x1, int y1, KeyedPoint pts[], int count)
{
    ...
    for (int ty = y0; ty < y1; ty += tileSize)
    {
        ...
        for (int tx = x0; tx < x1; tx += tileSize)
        {
            ...
            for (int i = 0; i < count; i++)
                insertKeyedPoint(pts, i, pts[i].pt, pts[i].temp + square(distanceBound(pts[i].pt.x, midX, radX)), pts[i].temp);
...
static void cellularTexTilesRect_SSE(Image &out, int x0, int y0, int x1, int y1, KeyedPoint pts[], int count)
{
    static int tileSize = 32;
    ...
    for (int ty = y0; ty < y1; ty += tileSize)
    {
        ...
        for (int tx = x0; tx < x1; tx += tileSize)
        {
            ...
            {
                Intens *dest = out.Row(y);
                __m128 curX = _mm_setr_ps(x0 + stepX * 0.0f, x0 + stepX * 1.0f, x0 + stepX * 2.0f, x0 + stepX * 3.0f);
...
private:
    ...
    void findNearestTwoR(const Point &pt, float &best, float &best2, int point1, float dist1, TreeNode *cur);
};
...
void Tree::findNearestTwoR(const Point &pt, float &best, float &best2, int point1, float dist1, TreeNode *cur)
{
    if (cur) // inner node
    {
...
static void cellularTexSpatialSubdR(Image &out, int x0, int y0, int x1, int y1, float radius, KeyedPoint pts[], int count)
{
    static const int minSize = 16;

    int hx = (x1 - x0) / 2; // diameter in x/y of subdivided rectangles
    int hy = (y1 - y0) / 2;
    float subRad = 0.5f * radius; // "radius" (half-extent) of subdivided rectangles
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        if (realIn >= 2 && hx >= minSize && hy >= minSize)
            cellularTexSpatialSubdR(out, sx0, sy0, sx0 + hx, sy0 + hy, subRad, pts, outCount);
        else
...
static void measure(CellularFunc func, const char *name, Image &out, const Point pts[], int count)
{
    printf("%20s: ", name);
    fflush(stdout);
